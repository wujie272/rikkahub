...
class McpManager(
    ...
    private suspend fun addClientLocked(config: McpServerConfig) {
        val resolvedConfig = ensureFreshToken(config)
        removeClientLocked(resolvedConfig) // Remove first
        cancelReconnect(resolvedConfig.id)
        reconnectAttempts[resolvedConfig.id] = 0
